...
# SIC panels (full 0–100 range, log-scaled y-axis)
...
alpha = 0.4
ax_div.hist(div_main, bins=hist_bins, density=True, alpha=alpha, color="k", label='15-31 July subsample')
ax_div.hist(div_case, bins=hist_bins, density=True, alpha=1, color="darkorange",histtype='step', label='Selected S2S pair', linewidth=3)
...
for ax_sic, main_vals, case_t0_vals, case_t1_vals, color, base_label in [
    ...
    plot_hist_with_kde(ax_sic, main_vals_clean, sic_bins, 'k', color, f"{base_label}", alpha=sic_alpha, plot_hist=True, plot_curve=False)
    plot_hist_with_kde(ax_sic, case_t0_clean, sic_bins, 'limegreen', color, rf"{base_label} t$_{{0}}$", alpha=sic_alpha, linestyle="-", plot_hist=True, plot_curve=False, histtype="step", linewidth=1.5)
    plot_hist_with_kde(ax_sic, case_t1_clean, sic_bins, 'magenta', color, rf"{base_label} t$_{{1}}$", alpha=sic_alpha, linestyle=":", plot_hist=True, plot_curve=False, histtype="step", linewidth=1.5)

    ax_sic.grid(True, linestyle=":", color="k", alpha=0.5)
    ax_sic.legend(loc='upper center', fontsize=FONTSIZE - 4)
    ax_sic.set_xlim([0, 100])
    ax_sic.set_yscale("log")
    ax_sic.set_ylim([1e-4, 1])
